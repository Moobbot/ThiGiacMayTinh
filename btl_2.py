...
# Khởi tạo client 1 lần thôi (không nên đặt trong vòng lặp)
...
def call_roboflow_api(img_path, max_retries=MAX_RETRIES):
    """Call Roboflow API with retry mechanism."""
...
def detect_oranges(combined_mask, frame, MIN_AREA):
    """Gửi frame đến Roboflow để phát hiện quả cam"""
    input_centroids = []
    detection_count = 0
                        ...
                        if dist < DIST_THRESH:
                            ...
                            if not data["counted"]:
                                ...
                                if inside:
                                    ...
                                    objects[obj_id][
                                        "id"
                                    ] = total_count  # Store the count ID
                            matched = True
                            break
                    if not matched:
                        objects[next_id] = {
